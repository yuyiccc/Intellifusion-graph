...
class GCNModelAE(Model):
    ...
    def _build(self):
        ...
        self.popularity = GraphConvolution(input_dim=self.config['hidden1'],
                                           output_dim=1,
                                           adj=self.adj,
                                           act=lambda x: x,
                                           if_bn=False,
                                           ...
                                           weight_init=self.config['weight_init'],
                                           logging=self.logging)(self.hidden1)
        self.popularity = tf.reshape(self.popularity, shape=[1, -1])

        self.productivity = GraphConvolution(input_dim=self.config['hidden1'],
                                             output_dim=1,
                                             adj=self.adj,
                                             act=lambda x: x,
                                             if_bn=False,
                                             ...
                                             weight_init=self.config['weight_init'],
                                             logging=self.logging)(self.hidden1)
        self.productivity = tf.reshape(self.productivity, shape=[-1, 1])

        self.z_mean = self.embeddings

        self.reconstructions, self.po, self.pro = InnerProductDecoder(num_nodes=self.num_nodes,
                                                                      act=lambda x: x,
                                                                      popularity_weight=self.popularity,
                                                                      productivity_weight=self.productivity,
